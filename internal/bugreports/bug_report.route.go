...
func CreateBugReport(db *sqlx.DB, dbName string, s3conn *storage.S3Connector) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if name == "" || email == "" || description == "" {
			response.Error(w, http.StatusBadRequest, "name, email and description are required")
			return
		}
		...
		for _, fr := range fileRoles {
			...
			if err != nil {
				response.Error(w, http.StatusInternalServerError, "reading file "+fr.field+": "+err.Error())
				return
			}
			...
			if s3conn != nil {
				...
				if err != nil {
					...
					if !isPathValid {
						response.Error(w, http.StatusInternalServerError, "invalid file path or name")
						return
					}
...
func GetBugReportByID(db *sqlx.DB, dbName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if errors.Is(reportErr, sql.ErrNoRows) {
			response.Error(w, http.StatusNotFound, "bug report not found")
			return
		}
		if reportErr != nil {
			response.Error(w, http.StatusInternalServerError, reportErr.Error())
			return
		}
		...
		response.OK(w, reportPayload{Report: report})
	}
}
...
func GetReportsCount(db *sqlx.DB, dbName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.TrimSpace(rawStatus) != "" {
			...
			if !ok {
				response.Error(w, http.StatusBadRequest, "invalid status. allowed values: new, in_review, resolved, closed")
				return
			}
			query += " WHERE status = ?"
...
func GetReportFileByFileID(db *sqlx.DB, dbName string, s3conn *storage.S3Connector) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if reportId == "" {
			response.Error(w, http.StatusBadRequest, "missing report id parameter")
			return
		}
		fileId := chi.URLParam(r, "file_id")
		if fileId == "" {
			response.Error(w, http.StatusBadRequest, "missing file id parameter")
			return
		}
...
func GetReportStatusByID(db *sqlx.DB, dbName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response.OK(w, map[string]string{"status": string(reportStatus)})
	}
}
...
func PatchBugReportStatus(db *sqlx.DB, dbName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			response.Error(w, http.StatusBadRequest, "unable to read request body: "+err.Error())
			return
		}
		...
		response.OK(w, map[string]string{"message": "status updated successfully"})
	}
}
...
func DeleteBugReportByID(db *sqlx.DB, dbName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response.OK(w, map[string]string{"message": "bug report deleted successfully"})
	}
}
