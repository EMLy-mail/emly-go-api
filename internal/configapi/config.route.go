...
func trackConfigFetch(r *http.Request, db *sqlx.DB, revision int64) {
	...
	clientID, err := updaterclient.Upsert(ctx, db, id)
	if err != nil {
...
func GetConfig(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to fetch published config")
			return
		}
...
func ValidateConfig(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(req.Document) == 0 {
			response.Error(w, http.StatusBadRequest, "document is required")
			return
		}
		...
		response.OK(w, map[string]interface{}{"valid": true, "warnings": revisionEchoWarnings(req.Document)})
	}
}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if status != "" {
			if !validRevisionStatus[status] {
				response.Error(w, http.StatusBadRequest, "status must be one of: draft, published, superseded")
				return
			}
		...
		if err := db.GetContext(r.Context(), &total,
			...
			response.Error(w, http.StatusInternalServerError, "failed to count revisions")
			return
		}
		...
		if err := db.SelectContext(r.Context(), &revisions, listQuery, listArgs...); err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to fetch revisions")
			return
		}
...
func GetConfigRevision(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := db.GetContext(r.Context(), &row,
			...
			response.Error(w, http.StatusInternalServerError, "failed to fetch revision")
			return
		}
		response.OK(w, row)
	}
...
func DeleteConfigRevision(db *sqlx.DB, cfg *config.Config) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to delete revision")
			return
		}
		if n, _ := res.RowsAffected(); n == 0 {
			...
			response.Error(w, http.StatusConflict, "only draft revisions can be deleted")
			return
		}
...
func CreateConfigRevision(db *sqlx.DB, cfg *config.Config, notifier ConfigNotifier) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(req.Document) > remoteconfig.MaxDocumentBytes {
			response.Error(w, http.StatusRequestEntityTooLarge, "document exceeds the 1 MiB size cap")
			return
		}
		...
		createdBy := session.Username(r, db)
		...
		if err := tx.GetContext(r.Context(), &maxRev, `SELECT MAX(revision) FROM remote_config_revisions FOR UPDATE`); err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to allocate revision")
			return
		}
		revision := int64(1)
		if maxRev.Valid {
		...
		if _, err := tx.ExecContext(r.Context(),
			...
			response.Error(w, http.StatusInternalServerError, "failed to create revision: "+err.Error())
			return
		}
		...
		if req.Publish {
			if err := publishRevisionTx(r.Context(), tx, revision); err != nil {
				response.Error(w, http.StatusInternalServerError, "failed to publish: "+err.Error())
				return
			}
			status = "published"
			now := time.Now().UTC()
...
func PublishConfigRevision(db *sqlx.DB, cfg *config.Config, notifier ConfigNotifier) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		switch status {
		case "published":
			response.Error(w, http.StatusConflict, "already published")
			return
		case "superseded":
			response.Error(w, http.StatusConflict, "superseded; use rollback to republish its content")
			return
		}
		...
		if err := tx.GetContext(r.Context(), &currentPublished,
			...
			response.Error(w, http.StatusInternalServerError, "failed to check current published revision")
			return
		}
		if currentPublished.Valid && revision <= currentPublished.Int64 {
			response.Error(w, http.StatusConflict, "a newer revision has been published since; create a new draft")
			return
		}
		...
		if err := db.GetContext(r.Context(), &row, `SELECT`+remoteConfigSelectCols+`FROM remote_config_revisions WHERE revision = ?`, revision); err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to fetch published revision")
			return
		}
		response.OK(w, row)
	}
...
func RollbackConfig(db *sqlx.DB, cfg *config.Config, notifier ConfigNotifier) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if req.To <= 0 {
			response.Error(w, http.StatusBadRequest, "to is required")
			return
		}
		...
		if err := tx.GetContext(r.Context(), &source,
			...
			if errors.Is(err, sql.ErrNoRows) {
				response.Error(w, http.StatusNotFound, "source revision not found (or is still a draft)")
				return
			}
			response.Error(w, http.StatusInternalServerError, "failed to fetch source revision")
			return
		}
		...
		if _, err := tx.ExecContext(r.Context(),
			...
			response.Error(w, http.StatusInternalServerError, "failed to create rollback revision: "+err.Error())
			return
		}

		if err := publishRevisionTx(r.Context(), tx, newRevision); err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to publish rollback: "+err.Error())
			return
		}
		if err := tx.Commit(); err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to commit")
		...
		if err := db.GetContext(r.Context(), &row, `SELECT`+remoteConfigSelectCols+`FROM remote_config_revisions WHERE revision = ?`, newRevision); err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to fetch rolled-back revision")
			return
		}
...
func PreviewConfig(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if hasRevision == hasDocument {
			response.Error(w, http.StatusBadRequest, "exactly one of revision or document is required")
			return
		}
		...
		if hasDocument {
			...
			if len(problems) > 0 {
				response.Error(w, http.StatusInternalServerError, "stored revision failed to re-validate")
				return
			}
			doc = parsed
		}
		...
		if req.Host.Now != nil && *req.Host.Now != "" {
			...
			if err != nil {
				response.Error(w, http.StatusBadRequest, "host.now must be RFC3339")
				return
			}
			host.Now = t
		}
