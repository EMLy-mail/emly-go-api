// Package ttlcache memoizes a handler payload for a short TTL and collapses
// callers racing on the same key into a single computation.
//
...
//
// It is the second of two defences there, not the only one: those queries read
...
//
// Only polled REST paths are cached. The WS stream (internal/stats) calls its
// fetchers directly, because it recomputes on an actual event or tick rather
// than on client demand - putting it behind this cache would buy nothing and
// could push a stale snapshot to a subscriber. Its own protection against
// recompute storms is coalescing, not memoization.
package ttlcache
...
// maxEntries bounds a Cache's key space. Keys are derived from query params, so
// a caller varying one on every request would otherwise grow the map without
// limit. Past this size the whole map is dropped rather than tracked with a real
// eviction policy: the only cost of a miss is rebuilding one value, and the
// endpoints using this cache have a handful of keys in practice.
const maxEntries = 512

// Cache is a TTL-bounded memoizer for values of type T. Construct with New; the
...
// New returns a Cache memoizing values for ttl. A non-positive ttl disables
// caching entirely - see Get.
