...
func GetUpdaterManifest(db *sqlx.DB, hub *statshub.Hub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if errors.Is(err, sql.ErrNoRows) {
			response.OK(w, models.UpdaterManifest{})
			return
		}
		if err != nil {
			// 5xx makes the client retry with backoff on its next cycle.
			response.Error(w, http.StatusInternalServerError, "failed to fetch updater release")
			return
		}
		...
		updaterclient.RecordEvent(r.Context(), db, r, hub, "manifest_check", updaterclient.ProductUpdater, uaVersion)
	}
}
...
func DownloadUpdater(db *sqlx.DB, s3conn *storage.S3Connector, s3Prefix string, hub *statshub.Hub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := db.GetContext(r.Context(), &filename,
			...
			response.Error(w, http.StatusNotFound, "updater release not found")
			return
		}
		...
		streamInstaller(w, r, rc, updaterclient.ProductUpdater, version, filename, info.Size)
		...
		defer cancel()
		updaterclient.RecordEvent(ctx, db, r, hub, "download", updaterclient.ProductUpdater, version)
	}
}
...
func ListUpdaterReleases(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := db.SelectContext(r.Context(), &releases,
			...
			response.Error(w, http.StatusInternalServerError, "failed to fetch updater releases")
			return
		}
...
func CreateUpdaterRelease(db *sqlx.DB, s3conn *storage.S3Connector, s3Prefix string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !updaterVersionPattern.MatchString(version) {
			response.Error(w, http.StatusBadRequest, "version must be semver without a leading 'v' (e.g. 1.5.0)")
			return
		}
		...
		if s := strings.TrimSpace(r.FormValue("published_at")); s != "" {
			...
			if err != nil {
				response.Error(w, http.StatusBadRequest, "published_at must be RFC3339")
				return
			}
			publishedAt = t.UTC()
		}
		...
		if isCurrent {
			if err := clearCurrentUpdaterFlag(r.Context(), tx, version); err != nil {
				response.Error(w, http.StatusInternalServerError, "failed to clear existing current release")
				return
			}
		}
		...
		if _, err := tx.ExecContext(r.Context(),
			...
			dbvalue.NullString(notesIT), dbvalue.NullString(notesEN), publishedAt,
		); err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to create updater release: "+err.Error())
			return
		}
...
func PatchUpdaterRelease(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if req.NotesIT != nil {
			...
			args = append(args, dbvalue.NullString(strings.TrimSpace(*req.NotesIT)))
		}
		if req.NotesEN != nil {
			setClauses = append(setClauses, "notes_en = ?")
			args = append(args, dbvalue.NullString(strings.TrimSpace(*req.NotesEN)))
		}
		...
		if err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to update updater release")
			return
		}
		...
		if err := db.GetContext(r.Context(), &updated,
			...
			response.Error(w, http.StatusInternalServerError, "failed to fetch updated updater release")
			return
		}
...
func DeleteUpdaterRelease(db *sqlx.DB, s3conn *storage.S3Connector, s3Prefix string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to delete updater release: "+err.Error())
			return
		}
		if n, _ := res.RowsAffected(); n == 0 {
			response.Error(w, http.StatusNotFound, "updater release not found")
