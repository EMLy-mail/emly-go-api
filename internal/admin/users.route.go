...
func CreateUser(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if body.Role != models.UserRoleAdmin && body.Role != models.UserRoleUser {
			response.Error(w, http.StatusBadRequest, "role must be 'admin' or 'user'")
			return
		}
		...
		id, err := generateUUID()
		if err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to generate id: "+err.Error())
			return
		}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if raw, ok := body["displayname"]; ok {
			...
			if err := json.Unmarshal(raw, &v); err != nil {
				response.Error(w, http.StatusBadRequest, "invalid displayname value")
				return
			}
		...
		if raw, ok := body["enabled"]; ok {
			var v bool
			if err := json.Unmarshal(raw, &v); err != nil {
				response.Error(w, http.StatusBadRequest, "invalid enabled value")
				return
			}
		...
		if len(fields) == 0 {
			response.Error(w, http.StatusBadRequest, "no updatable fields provided")
			return
		}
