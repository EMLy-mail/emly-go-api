...
func ValidateSession(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		sessionID := r.Header.Get(session.Header)
		if sessionID == "" {
			response.Error(w, http.StatusUnauthorized, "missing "+session.Header+" header")
			return
		}
...
func LogoutSession(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		sessionID := r.Header.Get(session.Header)
		if sessionID == "" {
			response.Error(w, http.StatusBadRequest, "missing "+session.Header+" header")
			return
		}
