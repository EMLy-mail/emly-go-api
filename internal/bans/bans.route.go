...
func ListBans(db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := db.SelectContext(r.Context(), &bans,
			...
			response.Error(w, http.StatusInternalServerError, "failed to list bans")
			return
		}
		if bans == nil {
			bans = []models.Ban{}
...
func CreateBan(db *sqlx.DB, bans BanReloader) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !models.ValidBanType(body.BanType) {
			response.Error(w, http.StatusBadRequest, "ban_type must be 'ip', 'hwid' or 'hostname'")
			return
		}
		...
		reason := dbvalue.NullString(strings.TrimSpace(body.Reason))
		...
		if err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to create ban")
			return
		}
		...
		if err := db.GetContext(r.Context(), &ban,
			...
			response.Error(w, http.StatusInternalServerError, "ban stored but could not be read back")
			return
		}
		...
		slog.WarnContext(r.Context(), "ban created",
			...
			"created_by", dbvalue.Deref(ban.CreatedBy))
...
func DeleteBan(db *sqlx.DB, bans BanReloader) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			response.Error(w, http.StatusBadRequest, "invalid ban id")
			return
		}
		...
		if errors.Is(err, sql.ErrNoRows) {
			response.Error(w, http.StatusNotFound, "ban not found")
			return
		}
		if err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to fetch ban")
			return
		}

		if _, err := db.ExecContext(r.Context(), `DELETE FROM bans WHERE id = ?`, id); err != nil {
			response.Error(w, http.StatusInternalServerError, "failed to delete ban")
			return
		}
