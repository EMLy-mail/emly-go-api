...
- `GET /v2/stats/summary` memoizes its payload behind a `ttlCache` (`statscache.go`), keyed by `product|window_minutes` and sized by `STATS_CACHE_TTL`. Dashboards poll it continuously while it reports 24h aggregates, so the queries run once per TTL rather than once per request; concurrent callers on a cold key collapse into one build. Anything added to that summary belongs in `buildStatsSummary`, behind the cache — not in the handler.
...
- Fleet stats: `STATS_CACHE_TTL` (default `30s`) — how long `GET /v2/stats/summary` may serve a memoized payload; `0` disables the cache
