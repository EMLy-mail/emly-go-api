...
# supersede semantics, nil-Hub safety, in internal/presencehub), the
# GET /v2/client/ws handshake (hello/identity/error framing and a full
# identity->presence-online round trip over a real listener, in
# internal/clientws), the daily log files (naming, day rotation, retention in
# internal/logfile) and the raw-event retention loop's disabled path
# (internal/eventprune)
go test ./...
